...
Day 16 的修复方向很直接：**跟 API 讲同一种语言**——请求里带上工具定义
...
flowchart LR
    ...
    LLM["LLM.complete(messages, tools=tools)<br/>（Day 16 新增 tools）"]
...
Day 16 新增 1 个文件（`final_answer.py`），改动 6 个已有文件。对照表：
...
Day 16 新增 8 个用例，全部使用 Fake LLM 与注入客户端，不访问网络。最代表
性的几组：
...
Day 17 会基于现在可用的真实模型链路编写端到端示例：单工具、多工具、工具
失败后恢复。到时可以直接运行
