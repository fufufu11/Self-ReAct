...
stateDiagram-v2
    ...
    错误观察 --> 异常完成: 不可恢复或预算耗尽
    解析失败 --> 解析重试: 该失败序列未达重试上限且仍有预算
    解析失败 --> 异常完成: 该失败序列达重试上限或预算耗尽
    解析重试 --> 推理决策: 回写稳定错误消息并消耗一步，成功后恢复重试资格
...
| 上下文压缩 | 完整消息列表、`ContextPolicy` | 压缩后的请求列表（必要时含摘要） | 超限时按整轮裁剪并回填规则式摘要 | 改写 `AgentState`、把记忆写进领域状态 |
...
解析失败必须成为显式结果，至少记录错误码、失败阶段和经安全截断的原始输出。R-02 起采用有界重试：解析失败时把稳定错误消息（只复用解析器的稳定中文说明，不泄漏原始输出）以 user 消息回写给模型、消耗一步预算，至多重试一次；重试仍失败或预算不足以发起重试时，以 `MODEL_OUTPUT_PARSE_ERROR` 终止。格式重试由循环控制器统一执行，不能形成解析重试的无限子循环，也不能用猜测修补动作。
...
当消息列表超过配置的字符窗口时，循环在每轮请求模型前应用 `ContextPolicy`（R-04）：以"整轮"为单位从旧到新裁剪（绝不拆开"助手请求调用工具 -> 工具结果"这对消息），并把被裁轮次压成固定模板的规则式摘要，以第二条 system 消息回填（Claude auto-compact 风格）。压缩只转换发给模型的请求，`AgentState.messages` 仍保留完整历史；system 提示词与首条任务消息永远保留且不计入预算。组件无状态、纯函数，相同历史永远得到相同请求，保证离线确定性，也满足领域模型 `extra="forbid"` 下"记忆不进 `AgentState`"的约束。这对应 LangChain/LangGraph 把记忆与上下文管理作为独立组件接入链路的做法，而不是把压缩逻辑写死在主循环里。
...
7. 上下文管理（R-04）：`ContextPolicy` 作为注入组件只在请求边界压缩消息，不改写 `AgentState`；摘要由固定规则生成，保持离线确定性。
