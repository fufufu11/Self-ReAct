...
聚焦任务后续可在任务文本中明确"给出小时级窗口即可"或继续提高预算/提示词
迭代（另立 Issue）。
...
- 第二轮后：开放任务 8 步预算内收敛（7/8）；**聚焦窗口任务仍 8/8 耗尽**——
  模型把"找出窗口"理解为逐分钟逼近边界，第 1 步已得小时桶答案却未收尾。
  后续可：任务文本明确"小时级窗口即可"、继续提示词迭代、或提高预算
  （另立 Issue）；工具层兜底（如 `log_query` 校验 `service`/`path` 参数）是
  defense-in-depth 方向，需单独评估（本 Issue 范围外）。
